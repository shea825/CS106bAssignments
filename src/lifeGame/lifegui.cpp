...
#include <util/random.h>    // for randomInteger
#include <util/strlib.h>    // for integerToString
#include <system/error.h>   // for error
...
void LifeGUI::initializeColors() {
    ...
    for (int age = 1; age <= kMaxAge; age++) {
        ostringstream oss;
        oss << "#";
        for (int primary : baseColor) {
            int contribution = scalePrimaryColor(primary, age);
...
void LifeGUI::updateFrame() {
    frame++;
    auto frameStr = "frame number: " + to_string(frame);
    window.setColor("WHITE");
    window.fillRect(0, 0, 140, 10);
    window.setColor("BLACK");
    window.drawString(frameStr, numRows, numColumns);
}

void LifeGUI::display(const Grid<state>& grid) {
    pause(50);
    window.setRepaintImmediately(false);
    for (int i = 0; i < grid.height(); ++i) {
        for (int j = 0; j < grid.width(); ++j) {
            drawCell(i, j, grid[i][j] == alive);
        }
    }
